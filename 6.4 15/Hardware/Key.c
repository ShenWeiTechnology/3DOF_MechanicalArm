...
void Key_Init(void)
{
	...
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_10|GPIO_Pin_11|GPIO_Pin_5|GPIO_Pin_6;
...
uint8_t Key_GetNum(void)
{
	...
	if (GPIO_ReadInputDataBit(GPIOB, GPIO_Pin_5) == 0)			//读PB11输入寄存器的状态，如果为0，则代表按键2按下
	{
		Delay_ms(20);											//延时消抖
	...
	if (GPIO_ReadInputDataBit(GPIOB, GPIO_Pin_6) == 0)			//读PB11输入寄存器的状态，如果为0，则代表按键2按下
	{
		Delay_ms(20);											//延时消抖
