...
uint8_t arm_go_to_point(float x, float y, float z)
{
	...
	if(flag_2==0)
	{
		FSUS_SetServoAngle(&usart2, 2, theta0+Angle_Compensation0   , 500, 0, 0); // 底座舵机ID2 
		FSUS_SetServoAngle(&usart2, 0, theta1+Angle_Compensation1	, 500, 0, 0); // 大臂舵机ID0 
		FSUS_SetServoAngle(&usart2, 1, theta2+Angle_Compensation2	, 500, 0, 0); // 小臂舵机ID1 
	}
