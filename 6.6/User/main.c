...
void Mode1_Run(void)
{
	float curAngle0_Mode1_Run;
	float curAngle1_Mode1_Run;
	float curAngle2_Mode1_Run;
	FSUS_QueryServoAngle(&usart2, 2, &curAngle0_Mode1_Run); //底座
	FSUS_QueryServoAngle(&usart2, 0, &curAngle1_Mode1_Run); //底座
	FSUS_QueryServoAngle(&usart2, 1, &curAngle2_Mode1_Run); //底座
	
	
	
    // 舵机2和3的角度状态变量（记录当前位置）
	static uint8_t servo0_angle_state = 0;
	static uint8_t servo1_angle_state = 0; // 0: -50.0度  1: 130.0度
...
//    }
    ...
    if(key == 3)
    {		
		if(servo0_angle_state==0)
		{
			FSUS_SetServoAngle(&usart2, 2, 0+Angle_Compensation0   , 800, 0, 0); // 底座舵机ID2
			FSUS_SetServoAngle(&usart2, 0, 90+Angle_Compensation1   , 800, 0, 0); // 中臂舵机ID0
			FSUS_SetServoAngle(&usart2, 1, 90+Angle_Compensation2   , 800, 0, 0); // 小臂舵机ID1	
			servo0_angle_state=1;
		}
		else
		{
			FSUS_SetServoAngle(&usart2, 2, 73+Angle_Compensation0   , 800, 0, 0); // 底座舵机ID2
			FSUS_SetServoAngle(&usart2, 0, 90+Angle_Compensation1   , 800, 0, 0); // 中臂舵机ID0
			FSUS_SetServoAngle(&usart2, 1, 90+Angle_Compensation2   , 800, 0, 0); // 小臂舵机ID1
			servo0_angle_state=0;
		}
		
		servo1_angle_state = 0;
    ...
    {		
		...
		if (Selected_Servo == 1)
        {
            if(servo1_angle_state == 0)
            {
				FSUS_WheelMoveTime(&usart2, 2, 0, 250, 4800);
				Delay_ms(4800);
				FSUS_SetServoAngle(&usart2, 2, 72   , 250, 0, 0); // 底座舵机ID2
				
//				FSUS_WheelMoveNCircle(&usart2, 2, 0, 400, 1);
//				FSUS_SetServoAngle(&usart2, 0, 120+Angle_Compensation1   , 800, 0, 0); // 中臂舵机ID0
//				FSUS_SetServoAngle(&usart2, 1, 90+Angle_Compensation2   , 800, 0, 0); // 小臂舵机ID1	
				
                servo1_angle_state = 1;
            }
            else
            {
				...
				FSUS_WheelMoveTime(&usart2, 2, 1, 300, 4800);
				Delay_ms(4800);
				FSUS_SetServoAngle(&usart2, 2, 74   , 300, 0, 0); // 底座舵机ID2
				
				
				
				
//				FSUS_WheelMoveNCircle(&usart2, 2, 1, 400, 1);
//                FSUS_WheelMoveTime(&usart2, 2, 1, 250, 10000);
//				FSUS_SetServoAngle(&usart2, 0, 90+Angle_Compensation1   , 800, 0, 0); // 中臂舵机ID0
//				FSUS_SetServoAngle(&usart2, 1, 90+Angle_Compensation2   , 800, 0, 0); // 小臂舵机ID1	
        ...
        if (Selected_Servo == 2)
        {
            if(servo2_angle_state == 0)
            {
                // 运动到130.0度
                FSUS_SetServoAngle(&usart2, 0, 120+Angle_Compensation1   , 800, 0, 0); // 中臂舵机ID0
				FSUS_SetServoAngle(&usart2, 1, 180+Angle_Compensation2   , 800, 0, 0); // 小臂舵机ID1
                servo2_angle_state = 1;
            }
            else
            {
                // 运动到-50.0度
                FSUS_SetServoAngle(&usart2, 0, 0+Angle_Compensation1   , 800, 0, 0); // 中臂舵机ID0
                servo2_angle_state = 0;
            }
        }
	...
	OLED_ShowNum(2, 5, (int)curAngle0_Mode1_Run, 2);
	OLED_ShowNum(2, 13, curAngle1_Mode1_Run, 2);
	OLED_ShowNum(3, 5, curAngle2_Mode1_Run, 2);
	
//    if(Servo1_State == SERVO1_STOP_RE || Servo1_State == SERVO1_STOP_FO)
//    {
//		OLED_ShowNum(2, 5, curAngle0_Mode1_Run, 2);
...
//	arm_go_to_point(50, 109+12, 83.0f+106.0f);
		...
		arm_go_to_point(0, 109+12+83, 83.0f+106.0f);
...
void Mode5_Run(void)
{
//			arm_go_to_point(0, 83+109+12, 106+83);
	//实时状态量的初始值用初始位置
	float curAngle0 = 0.0;
